                    ...
                    data.forEach(article => {
                        ...
                        articleElement.innerHTML = `
                            <div class="card-thumb">
                                <img src="${article.thumbnail || '/static/img/article-placeholder.svg?id=' + (article.id || 'default')}" alt="文章缩略图" onerror="this.onerror=null;this.src='/static/img/article-placeholder.svg';">
                            </div>
    ...
    function loadPhotos() {
        fetch('/api/photos')
            ...
            .then(data => {
                ...
                if (galleryGrid) {
                    // 仅当接口确实返回了图片时才替换内容。
                    // 否则保留 gallery.html 里的静态占位图，避免页面空白。
            ...
            articles.forEach(article => {
                ...
                el.innerHTML = `
                    <div class="latest-article-thumb">
                        <img src="${article.thumbnail || '/static/img/article-placeholder.svg?id=' + (article.id || 'default')}" alt="缩略图" onerror="this.onerror=null;this.src='/static/img/article-placeholder.svg';">
                    </div>
...
function startCountdown() {
    ...
    weiboCountdownInterval = setInterval(() => {
        ...
        timerEl.textContent = `热搜将于 ${minutes}分${seconds}秒后刷新`;
    }, 1000);
}
