let articleId = null;
var imagePreviewInited = false;
var imagePreviewScale = 1;
var IMAGE_PREVIEW_SCALE_MIN = 1;
var IMAGE_PREVIEW_SCALE_MAX = 5;
var IMAGE_PREVIEW_SCALE_STEP = 0.15;
var imagePreviewTranslateX = 0;
var imagePreviewTranslateY = 0;
...
function __initArticleDetailPage() {
    ...
    fetch('/api/articles/' + id)
        ...
        .then(function(data) {
            ...
            if (data.thumbnail) {
                ...
                bindPreviewForImage(document.getElementById('adCoverImg'));
            }
...
function initImagePreview() {
    ...
    modal.id = 'adImagePreview';
    modal.className = 'ad-image-preview';
    modal.innerHTML =
        '<button class="ad-image-preview-close" id="adImagePreviewClose" aria-label="关闭预览">' +
            '<i class="fa-solid fa-xmark"></i>' +
        '</button>' +
        '<img id="adImagePreviewImg" src="" alt="预览图片">';
    ...
    function getPreviewImg() {
        if (!previewImg) {
            previewImg = document.getElementById('adImagePreviewImg');
        }
        return previewImg;
    }
    ...
    function applyPreviewTransform() {
        ...
        if (!img) return;
        img.style.transform =
            'translate(' + imagePreviewTranslateX + 'px, ' + imagePreviewTranslateY + 'px) scale(' + imagePreviewScale + ')';
    ...
    function startDrag(clientX, clientY) {
        ...
        dragStartX = clientX;
        dragStartY = clientY;
        dragStartTranslateX = imagePreviewTranslateX;
        dragStartTranslateY = imagePreviewTranslateY;
        applyPreviewTransform();
    }

    function moveDrag(clientX, clientY) {
        if (!dragging) return;
        imagePreviewTranslateX = dragStartTranslateX + (clientX - dragStartX);
        imagePreviewTranslateY = dragStartTranslateY + (clientY - dragStartY);
        applyPreviewTransform();
    }
    ...
    modal.addEventListener('click', function(e) {
        if (e.target === modal || e.target.id === 'adImagePreviewClose' || e.target.closest('#adImagePreviewClose')) {
            closePreview();
        }
    });
    ...
    modal.addEventListener('wheel', function(e) {
        ...
        e.preventDefault();
        if (e.deltaY < 0) {
            imagePreviewScale = Math.min(IMAGE_PREVIEW_SCALE_MAX, imagePreviewScale + IMAGE_PREVIEW_SCALE_STEP);
        } else {
            imagePreviewScale = Math.max(IMAGE_PREVIEW_SCALE_MIN, imagePreviewScale - IMAGE_PREVIEW_SCALE_STEP);
        }
        if (imagePreviewScale <= 1) {
            imagePreviewTranslateX = 0;
            imagePreviewTranslateY = 0;
    ...
    var imgEl = getPreviewImg();
    if (imgEl) {
        imgEl.addEventListener('pointerdown', function(e) {
            if (!modal.classList.contains('show') || imagePreviewScale <= 1) return;
            e.preventDefault();
        ...
        imgEl.addEventListener('touchend', endDrag, { passive: true });
        imgEl.addEventListener('touchcancel', endDrag, { passive: true });
    }
...
function openImagePreview(src, alt) {
    if (!src) return;
    var modal = document.getElementById('adImagePreview');
    var img = document.getElementById('adImagePreviewImg');
    if (!modal || !img) return;
...
function bindPreviewForImage(img) {
    if (!img) return;
    img.style.cursor = 'zoom-in';
...
function bindPreviewForBodyImages() {
    var body = document.getElementById('adBody');
    if (!body) return;
    var images = body.querySelectorAll('img');
    for (var i = 0; i < images.length; i++) {
        bindPreviewForImage(images[i]);
    }
}


if (document.readyState === 'loading') {
    window.addEventListener('DOMContentLoaded', __initArticleDetailPage, { once: true });
} else {
    __initArticleDetailPage();
}
